...
ptt_post <- read_csv("../../../machine_learning/ptt_post_train.csv")

#F1+F2+F3+F5
df <- ptt_post %>% select(comment:dislike, type, reply:title_nu, 
                          pos_word, neg_word, 
                          反核:火力,
                          topic1:topic7,
                          stance) %>% na.omit()
...
df[,11:35] <- lapply(df[,11:35], factor)
...
#F1+F2+F3+F5
df <- ptt_post %>% 
  select(comment:dislike, type, reply:title_nu, 
                          pos_word, neg_word, 
                          反核:火力,
                          topic1:topic7,
                          stance) %>% na.omit() %>% 
  mutate(
    type.positive = ifelse(type=="positive", 1, 0),
    type.negative = ifelse(type=="negative", 1, 0),
    type.neutrl = ifelse(type=="neutral", 1, 0)
    ) %>% select(-type)
